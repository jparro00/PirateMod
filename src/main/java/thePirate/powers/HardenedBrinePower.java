...
public class HardenedBrinePower extends AbstractPower implements CloneablePowerInterface, OnUseEnergy {
    ...
    public static final String POWER_ID = PirateMod.makeID("HardenedBrinePower");
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String NAME = powerStrings.NAME;
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;


    public HardenedBrinePower(final int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public AbstractPower makeCopy() {
        return new HardenedBrinePower(amount);
    }

    @Override
    public void onUseEnergy(int e) {
        if (e > 0)
            addToBot(new GainBlockAction(owner, amount * e));
